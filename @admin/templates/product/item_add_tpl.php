...
function get_main_danhmuc($id=0)
	{		
		global $d;
		$temp = explode(',',$id);

		$d->reset();
		$sql="select id,ten from table_product_danhmuc where type='sanpham' order by stt asc,id desc";
		$d->query($sql);
		$row = $d->result_array();
		
		$str='<select id="id_danhmuc" name="id_danhmuc[]" multiple="multiple">';
...
function get_main_list($id=0)
	{		
		global $d;
		$temp = explode(',',$id);

		$d->reset();
		$sql="select id,ten from table_product_list where type='sanpham' order by stt asc,id desc";
		$d->query($sql);
		$row = $d->result_array();
		
		$str='<select id="id_list" name="id_list[]" multiple="multiple">';
		for($i=0;$i<count($row);$i++)
		{
			if($temp){	
