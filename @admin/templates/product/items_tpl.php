...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
      ...
      <tr>
        ...
        <td class="tb_data_small ">Danh mục cấp 1</td>
        <td class="tb_data_small">Danh mục cấp 1</td>
    ...
    <tbody>
        ...
        <td align="center" style="width:200px;">
			<?php
				$arr_danhmuc = explode(',',$items[$i]['id_danhmuc']);
			?>  
            <?php foreach($arr_danhmuc as $k=>$v) { ?> 
			   <?php
                    $d->reset();
                    $sql = "select ten from table_product_danhmuc where id='".$v."'";
		...
		<td align="center" style="width:200px;">
			<?php
				$arr_list = explode(',',$items[$i]['id_list']);
			?>  
            <?php foreach($arr_list as $k=>$v) { ?> 
			   <?php
                    $d->reset();
                    $sql = "select ten from table_product_list where id='".$v."'";
                    $result = mysql_query($sql);
                    $item_list = mysql_fetch_array($result);
                    echo @$item_list['ten'].',';
               ?>
